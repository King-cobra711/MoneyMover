...
function App() {
    ...
    if(aSendValue === 0 || aSendValue === "0"){
      ...
      const request = await fetch("http://localhost:3001/moneyToB", {
        ...
        if(res.status === 200){
          ...
          setShowAddTake(true);
          setTimeout(()=>{setShowAddTake(false)}, 1000);
          setASendValue(0);
          setBSendValue(0);
  ...
  return (
    ...
    <ReactStrap.Container>
      <ReactStrap.Row>
        <ReactStrap.Col id="borderR" className="accountName"><h3>Account A</h3></ReactStrap.Col>
        <ReactStrap.Col id="borderL" className="accountName"><h3>Account B</h3></ReactStrap.Col>
      </ReactStrap.Row>
      <ReactStrap.Row>
        <ReactStrap.Col id="balance" className="borderR">Balance: ${balanceA}{showAddTake ? (<p id="aside" className={Asend ? ("neg") : ("pos")}>{Asend ? ("- $" + addTake) : ("+ $" + addTake)}</p>) : (<p className="noVis"></p>)}</ReactStrap.Col>
        <ReactStrap.Col style={{position: "relative"}} id="balance" className="borderL">Balance: ${balanceB}{showAddTake ? (<p id="bside" className={Bsend ? ("neg") : ("pos")}>{Bsend ? ("- $" + addTake) : ("+ $" + addTake)}</p>) : (<p className="noVis"></p>)}</ReactStrap.Col>
      </ReactStrap.Row>
      <ReactStrap.Row>
        <ReactStrap.Col className="borderR">
          <ReactStrap.Form>
            <ReactStrap.Form.Group controlId="AmountA">
              <ReactStrap.Form.Control onBlur={()=> setBSendValue(0)} onChange={(event) => setASendValue(event.target.value)} type="number" placeholder="Enter amount..."></ReactStrap.Form.Control>
              <p className="error">{Aerror}</p>
              <ReactStrap.Button className="mt-10" onClick={() => submitA()}  variant="outline-primary">Send to B</ReactStrap.Button>
        ...
        <ReactStrap.Col className="borderL">
          <ReactStrap.Form>
            <ReactStrap.Form.Group controlId="AmountB">
              <ReactStrap.Form.Control onBlur={()=> setASendValue(0)} onChange={(event) => setBSendValue(event.target.value)} type="number" placeholder="Enter amount..."></ReactStrap.Form.Control>
              <p className="error">{Berror}</p>
            </ReactStrap.Form.Group>
            <ReactStrap.Button className="mt-10" onClick={() => submitB()} variant="outline-primary">Send to A</ReactStrap.Button>
          </ReactStrap.Form>
        </ReactStrap.Col>
      </ReactStrap.Row>
      <ReactStrap.Row>
        <ReactStrap.Col style={{paddingTop: "1.6em"}} className="borderR">
          <ReactStrap.Button onClick={()=>toggleA()} variant="light">Transaction History</ReactStrap.Button>
          <Collapse isOpen={isOpenA}>
        <Card>
          <CardBody>
          {Ahistory.length > 0 ? (Ahistory.map((val, key) => {
          return (
            <div>{val >= 0 ? (<p className="historyPos">+ ${val}</p>) : (<p className="historyNeg">- ${(val * -1)}</p>)}</div>
          )
        })) : (<p>N/A</p>)}
          </CardBody>
        </Card>
        ...
        <ReactStrap.Col style={{paddingTop: "1.6em"}}>
          <ReactStrap.Button onClick={()=>{toggleB()}} variant="light">Transaction History</ReactStrap.Button>
          <Collapse isOpen={isOpenB}>
        <Card>
          <CardBody>
            {Bhistory.length > 0 ? (Bhistory.map((val, key) => {
          return (
            <div>{val >= 0 ? (<p className="historyPos">+ ${val}</p>) : (<p className="historyNeg">- ${(val * -1)}</p>)}</div>
          )
        })) : (<p>N/A</p>)}
